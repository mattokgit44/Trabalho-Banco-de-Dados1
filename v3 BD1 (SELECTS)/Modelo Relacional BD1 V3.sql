/* https://github.com/mattokgit44/Trabalho-Banco-de-Dados1/tree/main/v2%20BD1 */
...
PRIMARY KEY(codigo_person, servidor),
FOREIGN KEY(codigo_person) REFERENCES CONTA(codigo_usuario) ON DELETE CASCADE,
FOREIGN KEY(servidor) REFERENCES JOGO(servidor)
);
...
INSERT INTO PERSONAGEM (codigo_person, servidor, vocacao, nivel, skills, money, status_person) VALUES (131102, "Kronera", "Sorcerer", 118, 82, 148202103, "OFFLINE"), (654327, "Ocera", "Druid", 284, 98, 284702103, "ONLINE"), (851351, "Ocera", "Knight", 52, 60, 300846, "ONLINE"), (358421, "Descubra", "Sorcerer", 42, 52, 100310, "ONLINE"), (657611, "Antica", "Sorcerer", 21, 8, 12450, "OFFLINE"), (843298, "Pacembra", "Paladin", 514, 112, 305210210, "ONLINE"), (321482, "Luminera", "Knight", 12, 4, 38652, "OFFLINE"), (984651, "Belobra", "Druid", 38, 40, 3845124, "ONLINE"), (476285, "Relembra", "Sorcerer", 8, 4, 22103, "OFFLINE"), (794612, "Relembra", "Paladin", 510, 113, 268420213, "OFFLINE");
...
INSERT INTO PAINEL_USUARIO (person_id, infos, acc_status, gerenciamento_conta) VALUES (131102 , "OFF", 2, "NORMAL"), (654327 , "ON", 1, "NORMAL"), (851351 , "ON", 1, "NORMAL"), (358421 , "ON", 1, "NORMAL"), (657611 , "OFF", 1, "NORMAL"), (843298 , "ON", 1, "NORMAL"), (321482 , "OFF", 1, "NORMAL"), (984651 , "ON", 1, "NORMAL"), (476285 , "OFF", 1, "NORMAL"), (794612 , "OFF", 1, "NORMAL");
...
-- --------------------------------------------------------------------------------------------------------
-- SELECTS-------------------------------------------------------------------------------------------------
-- EMAIL DE TODOS OS JOGADORES PREMIUM
SELECT email
FROM JOGADOR
WHERE codigo_usuario IN(SELECT codigo_usuario 
						FROM CONTA 
						WHERE status_acc = "PREMIUM");
-- PERSONAGENS DOS SERVIDORES COM MAIS DE 50 MISSOES LIBERADAS QUE ESTÃO ONLINES
SELECT codigo_person 
FROM PERSONAGEM 
WHERE servidor IN(SELECT servidor 
				  FROM JOGO 
                  WHERE status_jogo = 1 
                  AND missoes > 50);
-- UNIAO DOS GAMEMASTER E DOS ADMINISTRADORES
SELECT * 
FROM GAMEMASTER 
UNION 
SELECT * 
FROM ADMINISTRADOR;
-- SALDO DO BANCO DA GUILDAS DAS 3 MELHORES GUILDAS
SELECT saldo, rank_guild 
FROM GUILDA G, GUILD_BANK B 
WHERE G.ID = B.n_banco 
AND G.rank_guild <= 3;
-- RANK DA GUILDA DOS PERSONAGENS DE NIVEL > 100
SELECT rank_guild 
FROM GUILDA
WHERE mestre_id IN (SELECT codigo_person 
					FROM PERSONAGEM 
                    WHERE nivel > 100);
-- LISTA DOS PERSONAGENS MONITORADOS POR GAMEMASTER
SELECT ID_jogador 
FROM FUNCIONARIO_MONITORA_JOGADOR
WHERE ID_funcionario IN (SELECT ID 
						 FROM GAMEMASTER);
-- ID E VOCAÇÃO DOS JOGADORES QUE USAM O "CAPACETE" HELMET OF THE MAD (HOTM)
SELECT vocacao, codigo_person 
FROM PERSONAGEM 
WHERE codigo_person IN (SELECT ref_person 
						FROM INVENTARIO 
                        WHERE capacete = "HOTM");
-- NOME DO JOGADOR QUE TEM UM PERSONAGEM QUE TEM UMA SKIN DE MOCHILA E GUARDA UM ITEN EQUIPAVEL
SELECT nome 
FROM JOGADOR 
WHERE codigo_usuario IN (SELECT ID 
						 FROM MOCHILA
                         WHERE skin <> "NB" 
                         AND ID IN (SELECT ID_mochila 
									FROM MOCHILA_TEM_ITENS 
                                    WHERE ID_item IN (SELECT ID 
													  FROM ITENS 
                                                      WHERE utilidade = "Equipavel")));
-- ID DO DONO E PESO DAS MOCHILAS COM ITENS QUE TEM MAIS DE 50 UNIDADES
SELECT ID, peso 
FROM MOCHILA 
WHERE ID IN(SELECT ID_mochila 
			FROM MOCHILA_TEM_ITENS 
            WHERE ID_item IN(SELECT ID 
							 FROM ITENS
                             WHERE unidades > 50));
-- NOME DE TODOS OS JOGADORES EXCETO OS DA VOCAÇÃO SORCERER
SELECT nome 
FROM JOGADOR J, PERSONAGEM P 
WHERE vocacao <> "Sorcerer" 
AND J.codigo_usuario = P.codigo_person;
-- --------------------------------------------------------------------------------------------------------
